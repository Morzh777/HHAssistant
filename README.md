...
Примеры настройки провайдеров:

YandexGPT (текст):
```typescript
// src/config/openai.config.ts (фрагмент)
export const OPENAI_CONFIGS = {
  COVER_LETTER: { provider: 'openai', model: 'gpt-5-mini' },
  VACANCY_ANALYSIS: { provider: 'openai', model: 'gpt-5-mini' },
...
export const GIGACHAT = {
  ...
  tokenEnv: 'GIGACHAT_TOKEN',
};
```

Локальная LLM (например, T-Pro 2.0 через OpenAI-совместимый сервер):
```typescript
export const LOCAL_LLM = {
  provider: 'openai-compatible',
  apiUrlEnv: 'LOCAL_LLM_API_URL', // например: http://localhost:11434/v1
  apiKeyEnv: 'LOCAL_LLM_API_KEY', // фиктивный ключ, если требуется
  model: 'tpro-2.0',
};
```
...
- Для всех сущностей (резюме, вакансии, сопроводительные письма, анализ вакансий) генерируются эмбеддинги через модель `text-embedding-3-small`.
...
### Импорт существующих файлов (опционально)
Исторические JSON/HTML данные из `job-agent/job-agent/data/` больше не используются. Планируется вспомогательный скрипт миграции в БД Prisma. До выхода скрипта рекомендуем работать с новыми данными, создаваемыми приложением.
